...
export default function PlantCard() {
  ...
  useNuiEvent<Plant>("sendPlantData", (data) => {
    if (Array.isArray(data) && data.length == 0) return setPlant(null);
    setPlant(data);
  });
  ...
  const imgName =
    "https://www.seriouseats.com/thmb/1Vl9G52_YL5XdiwK78ylIlQyYfY=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/20240717-BoilingCorn-AmandaSuarez-SEA-f80166320b364edbbea05d65e538e270.jpg";
    ...
    <div className="main">
      ...
      <div className="proggresBarOut">
        <div
          ...
          style={{
            width: `${Math.max(0, Math.min(100, Number(health) || 0))}%`,
          }}
        >
          <p className="proggresBarText">
            {health}
            <span className="szazalek">%</span>
          </p>
        </div>
      </div>
      ...
      <div className="proggresBarOut">
        <div
          className="proggresBar"
          style={{
            width: `${Math.max(0, Math.min(100, Number(water) || 0))}%`,
          }}
        >
          <p className="proggresBarText">
            {water}
